...
def build_csv_entries(existing_by_url):
    ...
    for i, row in enumerate(deduped, 1):
        ...
        if url in existing_by_url:
            url_to_entry[url] = existing_by_url[url]
            skipped += 1
...
def main():
    ...
    if Path(INDEX_FILE).exists():
        ...
        print(f"Loaded {len(existing_by_url)} cached entries from {INDEX_FILE}\n")

    all_entries = []

    print("=" * 60)
    print("SOURCE 1: formulary_links.csv")
    print("=" * 60)
    all_entries += build_csv_entries(existing_by_url)

    print("=" * 60)
    print("SOURCE 2: Caremark (BCBS AR Commercial)")
    print("=" * 60)
    all_entries += build_caremark_entries(existing_by_url)

    print("=" * 60)
    print("SOURCE 3: UHC Commercial")
    print("=" * 60)
    all_entries += build_uhc_entries(existing_by_url)
    ...
    print("=" * 60)
    print(f"✅  {INDEX_FILE} written ({size_mb:.1f} MB, {len(all_entries):,} total entries)")
    print(f"    Deploy alongside app.py and restart Streamlit.")
    print("=" * 60)
