...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        if is_tricare:
            ...
            if _multi_entries:
                ...
                if best_entry:
                    ...
                    pa_criteria_link = (
                        f'<div style="display:block;width:100%;"><details style="display:inline-block;vertical-align:middle;">'
        ...
        cards_html += (
            ...
            '<div class="plan-label" style="flex-wrap:wrap;">'
    ...
    for _pa_plan_key, _pa_entries in _pa_entry_lookup.items():
        ...
        _pa_only_cards[_display_name.lower()] = (
            ...
            f'<div class="plan-label" style="flex-wrap:wrap;"><span style="{_pa_only_style}">{_display_name}</span>'
            f'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{_pa_link}</div>'
            '</div>'
        )

    # Merge PA-only cards into the main cards_html in sorted order
    # Build a sorted list of (sort_key, card_html) for all PA-only cards
    # Insert them into cards_html by rebuilding as a list
    if _pa_only_cards:
        # Split existing cards_html into individual cards
        import re as _re2
        _existing = _re2.findall(r'<div class="result-card[^"]*">.*?</div>\s*</div>', cards_html, _re2.DOTALL)
        # Extract sort keys from existing cards
        _all_cards = []
        for _c in _existing:
            _m = _re2.search(r'(?:href="[^"]*">|<span[^>]*>)([^<]+)', _c)
            _sort_key = _m.group(1).strip().lower() if _m else ''
            _all_cards.append((_sort_key, _c))
        for _k, _c in _pa_only_cards.items():
            _all_cards.append((_k, _c))
        _all_cards.sort(key=lambda x: x[0])
        cards_html = ''.join(c for _, c in _all_cards)
