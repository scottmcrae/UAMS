...
def parse_query(raw_query):
    """
    Split up to 4 words into plan filter and drug keyword.
    Words found in plan names (column D) are plan words; others are drug words.
    Consecutive plan words are merged; consecutive drug words are merged.
    """
    ...
    while i < len(words):
        if tags[i] == 'P':
            group = [words[i]]
            while i + 1 < len(words) and tags[i + 1] == 'P':
                i += 1
                group.append(words[i])
            plan_parts.append(' '.join(group))
        else:
            group = [words[i]]
        ...
        i += 1
    return ' '.join(plan_parts), ' '.join(drug_parts)
...
if go and keyword.strip():
    # ── Parse input: split into plan filter + drug keyword ────────────────────
    plan_filter, drug_term = parse_query(keyword.strip())

    # Fallback: if no drug term found, treat entire input as drug search
    if not drug_term and plan_filter:
    ...
    # Apply plan filter — ALL filter words must appear in plan name
    if plan_filter:
