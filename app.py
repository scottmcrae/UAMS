...
if go and keyword.strip():
    # ── Parse input: split into drug term and optional plan filter ────────────
    # Strategy: try each word as the drug; words that appear in plan names are plan filters
    raw_input = keyword.strip()
    words = raw_input.split()

    # Identify plan-filter words (words that loosely match any plan name)
    all_plan_names = " ".join(e.get("plan_name","").lower() for e in entries)
    plan_filter_words = [w for w in words if w.lower() in all_plan_names and len(w) > 2]
    drug_words = [w for w in words if w not in plan_filter_words]

    # Drug term = everything that's not a plan filter word
    drug_term = " ".join(drug_words).strip() if drug_words else raw_input
    plan_filter = " ".join(plan_filter_words).strip().lower() if plan_filter_words else ""

    # ── Run search ────────────────────────────────────────────────────────────
    results = search_entries(entries, drug_term, "All Plan Groups", "All Payers")

    # Apply plan filter if detected
    if plan_filter:
        results = [r for r in results if all(
            w in (r.get("plan_name","") or "").lower()
            for w in plan_filter.split()
        )]
