...
if not go or not keyword.strip():
    with st.expander("📋 View all indexed formularies"):
        st.dataframe(pd.DataFrame([{
            ...
            "URL":  e.get("url",""),
        } for e in sorted(entries, key=lambda e: e.get("plan_name","").lower())]), use_container_width=True)
    st.stop()
...
for r in (found_results + [] + []):
    ...
    <div class="{card_class}">
      ...
      <div style="font-size:0.72rem;color:#888;margin-top:4px;">💡 Once open, press <strong>Ctrl+F</strong> (Windows) or <strong>⌘+F</strong> (Mac) to jump to the drug name.</div>
