...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        # Tricare: derive status from which list the entry URL points to
        if is_tricare and r.get("found"):
            status_label = tricare_status_from_url(r.get("url",""))
            tier_html = f'<span style="font-size:0.85rem;color:var(--muted);margin-left:8px;">{status_label}</span>'
        else:
