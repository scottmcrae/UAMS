...
def extract_st(text, keyword):
    """Return True if Step Therapy is required near the keyword match."""
    pattern = re.compile(re.escape(keyword.strip()), re.IGNORECASE)
    for m in pattern.finditer(text):
        after = text[m.end() : m.end() + 300]
        first_lines = '\n'.join(after.split('\n')[:4])
        if re.search(r'\bST\b|step therapy', first_lines, re.IGNORECASE):
            return True
    return False
...
if go and keyword.strip():
    # ── Parse input: drug term + optional plan filter (either order) ──────────
    raw = keyword.strip()
    ...
    if len(parts) == 2:
        # Try both orderings: check which part returns drug results
        word_a, word_b = parts[0].strip(), parts[1].strip()
        results_a = search_entries(entries, word_a, "All Plan Groups", "All Payers")
        has_drug_a = any(r["found"] for r in results_a)
        if has_drug_a:
            drug_term   = word_a
            plan_filter = word_b.lower()
        else:
            drug_term   = word_b
            plan_filter = word_a.lower()

    # ── Run search ────────────────────────────────────────────────────────────
    results = search_entries(entries, drug_term, "All Plan Groups", "All Payers")

    # Apply plan filter if present (any filter word must appear in plan name)
    if plan_filter:
        filter_words = plan_filter.split()
    ...
    for r in (found_results + [] + []):
        ...
        cards_html += (
            ...
            '<div class="plan-label">' + plan_link + ' &nbsp; ' + tag + tier_html + pa_html + st_html + '</div>'
            '</div>'
        )
...
with st.expander("View all formularies"):
    ...
    _csv_plans = []
    _seen_tricare = False
    _csv_path = Path("formulary_links.csv")
    if _csv_path.exists():
        with open(_csv_path, newline="", encoding="utf-8-sig", errors="replace") as _f:
            for _row in _csv.reader(_f):
                if len(_row) >= 5 and _row[4].strip().startswith("http"):
                    _name = _row[3].strip()
                    _url  = _row[4].strip()
                    if "tricare" in _name.lower():
                        if _seen_tricare:
                            continue
                        _seen_tricare = True
                        _url = "https://www.express-scripts.com/frontend/open-enrollment/tricare/fst/#/"
                    _csv_plans.append({"plan_name": _name, "url": _url})
    _display_plans = sorted(_csv_plans, key=lambda x: x["plan_name"].lower()) if _csv_plans else sorted(entries, key=lambda e: e.get("plan_name","").lower())
    rows_html = "".join(
...
with st.expander("Prior Authorizations"):
    import csv as _csv2
    _pa_plans = []
    _pa_csv_path = Path("formulary_links.csv")
    if _pa_csv_path.exists():
        with open(_pa_csv_path, newline="", encoding="utf-8-sig", errors="replace") as _pf:
            for _row in _csv2.reader(_pf):
                if len(_row) >= 6 and _row[5].strip() and _row[5].strip().startswith("http"):
                    _pa_plans.append({"plan_name": _row[3].strip(), "pa_url": _row[5].strip()})
    _pa_plans.sort(key=lambda x: x["plan_name"].lower())
    pa_rows_html = "".join(
