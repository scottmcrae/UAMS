...
<div class="header-bar">
  ...
  <p>Search across all payer formulary PDFs instantly — no downloads, no waiting.</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.header("⚙️ Options")
    all_groups = sorted(set(e["plan_group"] for e in entries if e.get("plan_group")))
    sel_group  = st.selectbox("Filter by Plan Group", ["All Plan Groups"] + all_groups)

    all_payers = sorted(set(e["payer"] for e in entries if e.get("payer")))
    if sel_group != "All Plan Groups":
        all_payers = sorted(set(
            e["payer"] for e in entries
            if e.get("plan_group") == sel_group and e.get("payer")
        ))
    sel_payer = st.selectbox("Filter by Payer", ["All Payers"] + all_payers)

    st.divider()
    show_not_found = st.checkbox("Show 'Not Found' results", value=False)
    show_errors    = st.checkbox("Show index errors", value=False)
...
if not go or not keyword.strip():
    ...
    with st.expander("📋 View all indexed formularies"):
        st.dataframe(pd.DataFrame([{
            "Plan Group": e.get("plan_group",""), "Payer": e.get("payer",""),
            "Plan Name":  e.get("plan_name",""),  "Status": e.get("status",""),
            "URL": e.get("url",""),
        } for e in entries]), use_container_width=True)
    st.stop()

# ── Run search ────────────────────────────────────────────────────────────────
results       = search_entries(entries, keyword, sel_group, sel_payer)
found_results = [r for r in results if r["found"]]
missed        = [r for r in results if not r["found"] and not r["search_error"]]
...
if found_results:
    ...
    out_df = pd.DataFrame([{"plan_group": r.get("plan_group",""), "payer": r.get("payer",""), "plan_name": r.get("plan_name",""), "url": r.get("url","")} for r in found_results])
    st.download_button("⬇️ Download results as CSV", data=out_df.to_csv(index=False).encode(),
        file_name=f"formulary_{keyword.strip().replace(' ','_')}.csv", mime="text/csv")
else:
    st.error(f'❌ **"{keyword.strip()}"** was not found in any formulary.')

st.divider()

for r in (found_results + (missed if show_not_found else []) + (errors if show_errors else [])):
