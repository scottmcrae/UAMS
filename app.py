            ...
            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    ...
                    if fmt == 'bcbs':
                        ...
                        if excl:
                            val = _cv(excl.group(1))
                            if not _empty(val): result_rows.append(('Exclusion Criteria', val))
                        req = re.search(r'(?:^|\n)Required\s+(.+?)\nMedical\s*(.*?)\nInformation\b', section, re.IGNORECASE|re.DOTALL)
                        if req:
                            val = _cv(req.group(1) + ' ' + req.group(2))
                            if not _empty(val): result_rows.append(('Required Medical Info', val))
                        age = re.search(r'(?:^|\n)Age\s+Restrictions?\n(.+?)\n(?:Prescriber|Coverage|Other|Part)', section, re.IGNORECASE|re.DOTALL)
                        if age:
                            val = _cv(age.group(1))
                            if not _empty(val) and not val.lower().startswith('prescriber'): result_rows.append(('Age Restrictions', val))

                    else:  # cigna_uhc
                        # Exclusion: "Exclusion [val]\nCriteria" (UHC) or "Exclusion Criteria\n" with no value (Cigna)
                        excl_split = re.search(r'(?:^|\n)Exclusion\s+(.+?)\nCriteria\b', section, re.IGNORECASE|re.DOTALL)
                        if excl_split and _cv(excl_split.group(1)).lower() not in ('criteria', ''):
                            val = _cv(excl_split.group(1))
                            if not _empty(val): result_rows.append(('Exclusion Criteria', val))
                        # Required Medical: Cigna "Required Medical Diagnosis\nInformation" → "Diagnosis"
                        #                   UHC   "Required [val]\nMedical [val]\nInformation"
                        ...
                        if req_cigna:
                            result_rows.append(('Required Medical Info', 'Diagnosis'))
                        elif req_split and _cv(req_split.group(1)).lower() not in ('medical','information',''):
                            val = _cv(req_split.group(1) + ' ' + req_split.group(2))
                            if not _empty(val): result_rows.append(('Required Medical Info', val))
                        elif req_inline:
                            val = _cv(req_inline.group(1))
                            if not _empty(val): result_rows.append(('Required Medical Info', val))
                        # Age
                        ...
                        if other:
                            val = _cv(other.group(1))
                            if not _empty(val) and not val.lower().startswith('indications'): result_rows.append(('Other Criteria', val))
                        # Off-Label: only if value isn't a label name
                        off = re.search(r'(?:^|\n)Off[\s\-]?Label\s+Uses?\n(.+?)(?=\n(?:Exclusion|Required|Age|Prescriber|Coverage|Other|Part|Prior)|\Z)', section, re.IGNORECASE|re.DOTALL)
                        if off:
                            val = _cv(off.group(1))
                            _label_names = ('exclusion','required','age','prescriber','coverage','other','part b','prerequisite')
                            if not _empty(val) and not any(val.lower().startswith(l) for l in _label_names):
                                result_rows.append(('Off-Label Uses', val))

                    # Always display all categories, even if empty
                    ALL_CATEGORIES = ['Exclusion Criteria', 'Required Medical Info', 'Age Restrictions', 'Other Criteria', 'Off-Label Uses', 'Part B Prerequisite']
                    result_dict = dict(result_rows)
