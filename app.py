...
if (fda_go or _search_mode == "fda") and keyword.strip():
    ...
    # Deduplicate: same drug name + same PDF URL = same entry
    _fda_seen = set()
    _fda_deduped = []
    for _d in _fda_matches:
        _key = (_d.get("drug_name", "").lower(), _d.get("pdf_url", ""))
        if _key not in _fda_seen:
            _fda_seen.add(_key)
            _fda_deduped.append(_d)
    _fda_matches = _fda_deduped
    ...
    with _results_placeholder.container():
        if not _fda_medguides:
            ...
            for _d in _fda_matches:
                ...
                _text_preview = ""
                if _text:
                    # Slice to INDICATIONS AND USAGE (scraper should have done this,
                    # but handle it here too as a display-time fallback)
                    import re as _re
                    _ind_m = _re.search(r'INDICATIONS\s+AND\s+USAGE', _text, _re.IGNORECASE)
                    _display_text = _text[_ind_m.start():] if _ind_m else _text
                    _preview_raw = _display_text[:800].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                    _text_preview = (
                        f'<details style="margin-top:6px;">'
                        f'<summary style="font-size:0.75rem;color:#548DBF;cursor:pointer;'
                        f'font-weight:600;list-style:none;">&#9654; Medication Guide text</summary>'
                        f'<div style="margin-top:6px;padding:10px 14px;background:#f8fafc;'
                        f'border-radius:6px;font-size:0.78rem;line-height:1.6;color:#1e3448;'
                        f'white-space:pre-wrap;max-height:400px;overflow-y:auto;">'
                        f'{_preview_raw}</div></details>'
                    )
