            ...
            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    ...
                    if fmt == 'bcbs':
                        ...
                        other = re.search(r'(?:^|\n)Other\s+Criteria\s*\n(.+?)\n(?:Part|Prerequisite|Off|Indications|Prior|\Z)', section, re.IGNORECASE|re.DOTALL)
                        result_rows.append(('Other Criteria', _cv(other.group(1)) if other else ''))

                        indic = re.search(r'(?:^|\n)Indications\s+(.+?)(?=\n(?:Off|Part|Prior)|\Z)', section, re.IGNORECASE|re.DOTALL)
                        result_rows.append(('Indications', _cv(indic.group(1)) if indic else ''))
