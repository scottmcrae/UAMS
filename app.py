...
_pa_text_lookup  = {}   # normalized plan_name -> single text (legacy plans)
_pa_entry_lookup = {}   # normalized plan_name -> list of {drug_label, text, url, source}
    ...
    with open(PA_INDEX_FILE, encoding="utf-8") as _paf:
        ...
        for _pae in _pa_index.get("entries", []):
            if _pae.get("text") and _pae.get("status") == "ok":
                _nkey = _normalize_plan(_pae["plan_name"])
                _src  = _pae.get("source", "")
                _is_multi = _src in ("caremark", "uhc_commercial") or bool(_pae.get("drug_label", ""))
                if _is_multi:
                    # Multi-entry plans: store list of entries
                    if _nkey not in _pa_entry_lookup:
                        _pa_entry_lookup[_nkey] = []
                    _pa_entry_lookup[_nkey].append({
                        "drug_label": _pae.get("drug_label", ""),
                        "text":       _pae["text"],
                        "url":        _pae.get("url", "") or _pae.get("pa_url", ""),
                    })
                else:
                    # Legacy single-text plans
                    _pa_text_lookup[_nkey] = _pae["text"]
        ...
        if is_tricare:
            ...
            if _multi_entries:
                ...
                for _entry in _multi_entries:
                    ...
                    if drug_term.lower() in _haystack:
                        # Score by how early the match appears in drug_label
                        _label_match = drug_term.lower() in _entry["drug_label"].lower()
                        _score = 2 if _label_match else 1
                        if _score > best_score:
                            best_score = _score
                            best_entry = _entry

                if best_entry:
                    # Find the relevant section within the text
                    ...
                    pa_criteria_link = (
                        ...
                        f'<summary style="display:inline;font-size:0.88rem;color:#856404;font-weight:600;cursor:pointer;text-decoration:none;border-bottom:1.5px solid #856404;padding-bottom:1px;">PA Criteria ▾</summary>'
                    ...
                    pa_criteria_link = f'<a href="{pa_criteria_url}" target="_blank" style="font-size:0.88rem;color:#856404;font-weight:600;text-decoration:none;border-bottom:1.5px solid #856404;padding-bottom:1px;margin-left:15px;">PA Criteria ↗</a>' if pa_criteria_url else ""
                ...
                if pa_criteria_text:
                    ...
                    if fmt == 'bcbs':
                        ...
                        if excl_split and _cv(excl_split.group(1)).lower() not in ('criteria', ''):
                            excl_val = _cv(excl_split.group(1))
                        ...
                        if req_cigna:
                            ...
                            req_val = _cv(req_split.group(1) + ' ' + req_split.group(2))
                        elif req_inline:
