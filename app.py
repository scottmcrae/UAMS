...
def _load_pa_index(filepath, multi_entry=False):
    ...
    with open(filepath, encoding="utf-8") as _f:
        ...
        for _e in _data.get("entries", []):
            if not (_e.get("text") and _e.get("status") == "ok"):
                continue
            ...
            if multi_entry:
                ...
                _pa_entry_lookup[_nkey].append({
                    ...
                    "text":       _e["text"],
                    "url":        _e.get("url", "") or _e.get("pa_url", ""),
                })
            else:
                _pa_text_lookup[_nkey] = _e["text"]
            ...
            if _multi_entries:
                ...
                if best_entry:
                    ...
                    if _section_start is not None:
                        ...
                        # End at the next drug's section (another drug name on its own line followed by FDA or newline)
                        _next = re.search(r'\n[A-Z][a-zA-Z].*?\nFDA-approved', _text[_section_start+1:], re.DOTALL)
                        _end = _section_start + 1 + _next.start() if _next else min(len(_text), _section_start + 3000)
                    else:
