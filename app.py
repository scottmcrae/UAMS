...
""", unsafe_allow_html=True)
    ...
    for r in (found_results + [] + []):
        ...
        is_tricare = "tricare" in (r.get("plan_group","") + r.get("payer","") + (r.get("plan_name") or "")).lower()
        display_url = TRICARE_URL if is_tricare else r.get("url","")
    ...
    <div class="{card_class}">
      <div class="plan-label"><a href="{display_url}" target="_blank" style="color:#1a1a1a;text-decoration:none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">{plan_label}</a> &nbsp; {tag}{tier_html}</div>
