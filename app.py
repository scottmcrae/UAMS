    ...
    for r in (found_results + [] + []):
        ...
        if is_tricare:
            ...
            if status_label != "Covered":
                continue
            tier_html = '<span style="font-size:0.8rem;color:var(--muted);margin-left:8px;">Covered</span>'
        else:
            ...
            if is_aetna and r.get("found") and r.get("text"):
                ...
                tier_html = f'<span style="font-size:0.8rem;color:var(--muted);margin-left:8px;">{code}</span>' if code else ""
            else:
                tier = extract_tier(r.get("text") or "", keyword) if r.get("found") and r.get("text") else None
                if tier:
                    tier_html = f'<span style="font-size:0.8rem;color:var(--muted);margin-left:8px;">{tier}</span>'
                else:
                    tier_html = ""
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: transparent; font-family: 'DM Sans', sans-serif; padding: 16px 0; }
  .card { width: 100%; background: #ffffff; border: 1px solid #e4dfd8; border-radius: 24px; overflow: hidden; position: relative; box-shadow: 0 4px 32px rgba(0,0,0,0.08); }
  .header { padding: 32px 40px 24px; border-bottom: 1px solid #ede8e2; position: relative; overflow: hidden; }
  .header::before { content: ''; position: absolute; top: -60px; right: -60px; width: 260px; height: 260px; background: radial-gradient(circle, rgba(99,153,34,0.10) 0%, transparent 70%); pointer-events: none; }
  .eyebrow { font-size: 10px; font-weight: 500; letter-spacing: 0.18em; text-transform: uppercase; color: #3B6D11; margin-bottom: 10px; }
  .title { font-family: 'DM Serif Display', serif; font-size: 28px; color: #1a1a1a; line-height: 1.15; margin-bottom: 10px; }
  .subtitle { font-size: 13px; font-weight: 300; color: #888278; line-height: 1.6; }
  .axis-bar { display: flex; align-items: center; gap: 10px; padding: 16px 40px 0; }
  .axis-line { flex: 1; height: 2px; background: linear-gradient(to right, #3B6D11, #A32D2D); border-radius: 2px; position: relative; }
  .axis-line::after { content: ''; position: absolute; right: -1px; top: -4px; width: 0; height: 0; border-left: 7px solid #A32D2D; border-top: 5px solid transparent; border-bottom: 5px solid transparent; }
  .axis-label { font-size: 10px; letter-spacing: 0.12em; text-transform: uppercase; color: #aaa49e; font-weight: 500; white-space: nowrap; }
  .tiers { padding: 12px 40px 32px; display: flex; flex-direction: column; gap: 8px; }
  .tier { display: grid; grid-template-columns: 56px 1fr auto; align-items: start; border-radius: 12px; overflow: hidden; border: 1px solid transparent; }
  .tier-num { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 16px 0; font-family: 'DM Serif Display', serif; font-size: 22px; line-height: 1; gap: 3px; }
  .tier-word { font-family: 'DM Sans', sans-serif; font-size: 8px; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; opacity: 0.6; }
  .tier-body { padding: 14px 16px; border-left: 1px solid rgba(0,0,0,0.06); }
  .tier-label { font-size: 13px; font-weight: 500; margin-bottom: 3px; }
  .tier-desc { font-size: 12px; font-weight: 300; line-height: 1.55; margin-bottom: 4px; }
  .tier-example { font-size: 10.5px; font-style: italic; opacity: 0.5; }
  .tier-cost { padding: 14px 14px 14px 0; display: flex; align-items: flex-start; justify-content: flex-end; }
  .cost-pill { font-size: 11px; font-weight: 500; padding: 4px 10px; border-radius: 20px; white-space: nowrap; }
  .t1 { background: #f0f7e8; border-color: #d4eab8; } .t1 .tier-num { background: #e4f2d0; color: #3B6D11; } .t1 .tier-label { color: #27500A; } .t1 .tier-desc { color: #557a30; } .t1 .cost-pill { background: #d4eab8; color: #3B6D11; }
  .t2 { background: #edf4fc; border-color: #c5ddf5; } .t2 .tier-num { background: #daeaf8; color: #185FA5; } .t2 .tier-label { color: #0c447c; } .t2 .tier-desc { color: #3a6e9e; } .t2 .cost-pill { background: #c5ddf5; color: #185FA5; }
  .t3 { background: #fdf6e8; border-color: #f5e0a8; } .t3 .tier-num { background: #faefd0; color: #854F0B; } .t3 .tier-label { color: #633806; } .t3 .tier-desc { color: #9a6820; } .t3 .cost-pill { background: #f5e0a8; color: #854F0B; }
  .t4 { background: #fdf0eb; border-color: #f5cebb; } .t4 .tier-num { background: #fae2d5; color: #993C1D; } .t4 .tier-label { color: #712B13; } .t4 .tier-desc { color: #a35030; } .t4 .cost-pill { background: #f5cebb; color: #993C1D; }
  .t5 { background: #fdf0f0; border-color: #f5c0c0; } .t5 .tier-num { background: #fad8d8; color: #A32D2D; } .t5 .tier-label { color: #791F1F; } .t5 .tier-desc { color: #a84040; } .t5 .cost-pill { background: #f5c0c0; color: #A32D2D; }
  .footer-note { margin: 0 40px 28px; padding: 14px 18px; background: #faf8f5; border: 1px solid #ede8e2; border-radius: 10px; display: flex; align-items: flex-start; gap: 12px; }
  .note-icon { width: 18px; height: 18px; border-radius: 50%; background: #ede8e2; border: 1px solid #d8d2ca; color: #999490; font-size: 11px; font-weight: 500; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; }
  .note-text { font-size: 11.5px; color: #999490; font-weight: 300; line-height: 1.65; }
  .note-text strong { color: #6b6560; font-weight: 500; }
</style>
</head>
<body>
<div class="card">
  <div class="header">
    <div class="eyebrow">Health Insurance Explained</div>
    <div class="title">Drug Tiers &amp; What You Pay</div>
    <div class="subtitle">Your plan groups every covered drug into tiers. Lower tiers = negotiated deals = lower cost to you.</div>
  </div>
  <div class="axis-bar">
    <div class="axis-label">Your cost</div>
    <div class="axis-line"></div>
  </div>
  <div class="tiers">
    <div class="tier t1">
      <div class="tier-num">1<span class="tier-word">Tier</span></div>
      <div class="tier-body"><div class="tier-label">Generic drugs</div><div class="tier-desc">Same active ingredient as the brand name. FDA-approved. Your plan loves these.</div><div class="tier-example">metformin · lisinopril · amoxicillin</div></div>
      <div class="tier-cost"><span class="cost-pill">$0 – $15</span></div>
    </div>
    <div class="tier t2">
      <div class="tier-num">2<span class="tier-word">Tier</span></div>
      <div class="tier-body"><div class="tier-label">Preferred brand-name</div><div class="tier-desc">Brand-name drugs your insurer has negotiated a deal with. Still a decent copay.</div><div class="tier-example">Lipitor · Zoloft · Synthroid</div></div>
      <div class="tier-cost"><span class="cost-pill">$30 – $60</span></div>
    </div>
    <div class="tier t3">
      <div class="tier-num">3<span class="tier-word">Tier</span></div>
      <div class="tier-body"><div class="tier-label">Non-preferred brand-name</div><div class="tier-desc">Works fine — your plan just didn't cut a deal with this manufacturer.</div><div class="tier-example">Brand your doctor prefers · newer name-brands</div></div>
      <div class="tier-cost"><span class="cost-pill">$60 – $100+</span></div>
    </div>
    <div class="tier t4">
      <div class="tier-num">4<span class="tier-word">Tier</span></div>
      <div class="tier-body"><div class="tier-label">Specialty drugs</div><div class="tier-desc">Complex conditions. Often 20–30% coinsurance instead of a flat copay. Prior auth common.</div><div class="tier-example">Humira · Enbrel · MS &amp; cancer drugs</div></div>
      <div class="tier-cost"><span class="cost-pill">20–30% cost</span></div>
    </div>
    <div class="tier t5">
      <div class="tier-num">5<span class="tier-word">Tier</span></div>
      <div class="tier-body"><div class="tier-label">Very high-cost specialty</div><div class="tier-desc">Highest cost-sharing. May require step therapy — proving cheaper options didn't work first.</div><div class="tier-example">Gene therapies · biologic infusions</div></div>
      <div class="tier-cost"><span class="cost-pill">Highest</span></div>
    </div>
  </div>
  <div class="footer-note">
    <div class="note-icon">i</div>
    <div class="note-text"><strong>Not all plans have 5 tiers</strong> — some use 3 or 4. Always look up your plan's <strong>formulary</strong> (the official drug list) to find the tier for your specific medication. Your doctor can request a <strong>formulary exception</strong> if you need a higher-tier drug.</div>
  </div>
</div>
</body>
</html>
