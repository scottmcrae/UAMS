...
def format_freetext_html(text):
    ...
    for line in lines:
        ...
        if not s:
            ...
            continue
        esc = _html.escape(s)
        # Detect hierarchy level by line prefix
        if re.match(r'^[A-Z]\.\s', s):          # A. B. — top section
            parts.append(f'<div style="margin:6px 0 2px 0;font-weight:600;color:#1e3448;">{esc}</div>')
        elif re.match(r'^\d+\.\s', s):            # 1. 2. — level 2
            parts.append(f'<div style="margin:4px 0 1px 16px;font-weight:600;">{esc}</div>')
        elif re.match(r'^[a-z]\.\s', s):          # a. b. — level 3
            parts.append(f'<div style="margin:2px 0 1px 32px;">{esc}</div>')
        elif re.match(r'^\(\d+\)\s', s):          # (1) (2) — level 4
            parts.append(f'<div style="margin:2px 0 1px 48px;">{esc}</div>')
        elif re.match(r'^\([a-z]\)\s', s):        # (a) (b) — level 5
            parts.append(f'<div style="margin:1px 0 1px 64px;">{esc}</div>')
        elif re.match(r'^[-•]\s', s):             # bullets
            parts.append(f'<div style="margin:1px 0 1px 48px;">{esc}</div>')
        elif s == '-AND-' or s == '-OR-':          # connectors
            parts.append(f'<div style="margin:2px 0 2px 48px;color:#856404;font-weight:500;">{esc}</div>')
        elif re.match(r'^©', s) or re.match(r'^Authorization will be', s, re.IGNORECASE):
            parts.append(f'<div style="margin:4px 0 1px 32px;color:#888;font-size:0.75rem;">{esc}</div>')
        else:                                      # continuation / wrapped line
            parts.append(f'<div style="margin:1px 0 1px 48px;">{esc}</div>')
    return ''.join(parts)
        ...
        if is_tricare:
            ...
            if _multi_entries:
                ...
                if best_entry:
                    ...
                    if _cov_m:
                        # Use Coverage Criteria as start — most useful content
                        _start = _cov_m.start()
                        _end = min(len(_text), _start + 3000)
                    ...
                    _snippet_html = format_freetext_html(_snippet)
                    ...
                    pa_criteria_link = (
                        ...
                        f'<div style="margin-top:8px;padding:14px 16px;background:#fffbf0;border-left:3px solid #e8913a;border-radius:0 6px 6px 0;max-height:380px;overflow-y:auto;box-sizing:border-box;">'
                        f'{_inner}'
                        f'</div></details>'
                    )
                else:
