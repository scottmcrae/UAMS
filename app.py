...
def format_freetext_html(text):
    ...
    def _reflow_background(m):
        ...
        joined = re.sub(r'(?<![.!?*])\n(?=[a-zA-Z(])', ' ', body)
        # Add a blank line between sentences (after ". " or ".\n")
        spaced = re.sub(r'\.\s+(?=[A-Z])', '.\n\n', joined)
        return header + spaced.strip()
    text = re.sub(
        r'\d+\.\s*Background:.*?(?=\n\d+\.\s*\w|\Z)',
                ...
                if _multi_entries:
                    ...
                    if best_entry:
                        ...
                        if _is_aetna_cpb:
                            ...
                            if _bg_m:
                                _start = _bg_m.start()
                                _end = _cont_m.start() if (_cont_m and _cont_m.start() > _bg_m.start()) else min(len(_text), _start + 6000)
                            elif _ind_m:
                                _start = _ind_m.start()
        ...
        for _pa_plan_key, _pa_entries in _pa_entry_lookup.items():
            ...
            for _pat in [
                ...
                if _sm:
                    ...
                    break
            if _bg_m:
                _section_start = _bg_m.start()
                _end = _cont_m.start() if (_cont_m and _cont_m.start() > _bg_m.start()) else min(len(_text), _section_start + 6000)
            elif _cov_m:
