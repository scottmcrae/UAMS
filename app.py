...
_pa_text_lookup = {}  # normalized plan_name -> criteria text
    ...
    with open(PA_INDEX_FILE, encoding="utf-8") as _paf:
        ...
        for _pae in _pa_index.get("entries", []):
            if _pae.get("text") and _pae.get("status") == "ok":
                _pa_text_lookup[_normalize_plan(_pae["plan_name"])] = _pae["text"]
                ...
                if pa_criteria_text:
                    ...
                    if fmt == 'bcbs':
                        for pat, label in [
                            ...
                            (r'Required Medical Information[ \t]+(.*)', 'Required Medical Info'),
                            (r'Age Restrictions[ \t]+(.*)', 'Age Restrictions'),
                            (r'Prescriber Restrictions[ \t]+(.*)', 'Prescriber Restrictions'),
                            (r'Coverage Duration[ \t]+(.*)', 'Coverage Duration'),
                            (r'Other Criteria[ \t]+(.*)', 'Other Criteria'),
                            (r'PA Indication Indicator[ \t]+(.*)', 'Indications'),
                            (r'Off-label Uses[ \t]+(.*)', 'Off-Label Uses'),
                            (r'Part B[ \t]+(.*)', 'Part B Prerequisite'),
                        ]:
                            m = re.search(pat, section, re.IGNORECASE)
                            val = _cv(m.group(1).split('\n')[0]) if m else ''
                    ...
                    pa_criteria_link = (
                        ...
                        f'<summary style="display:inline;font-size:0.88rem;color:#856404;font-weight:700;cursor:pointer;text-decoration:none;border-bottom:1.5px solid #856404;padding-bottom:1px;">PA Criteria ▾</summary>'
                    ...
                    pa_criteria_link = (
                        f' <a href="{pa_criteria_url}" target="_blank" style="font-size:0.88rem;color:#856404;font-weight:700;text-decoration:none;border-bottom:1.5px solid #856404;padding-bottom:1px;margin-left:15px;">PA Criteria ↗</a>'
                    )
            else:
                pa_criteria_link = ""
