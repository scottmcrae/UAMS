...
_btn_cols = st.columns([1, 1, 1, 4])
with _btn_cols[0]:
    st.link_button("💊 GoodRx", "https://www.goodrx.com/", use_container_width=True)
with _btn_cols[1]:
    st.link_button("🔬 OpenEvidence", "https://www.openevidence.com", use_container_width=True)
with _btn_cols[2]:
    if _kw:
        st.code(_kw, language=None)
if _kw:
    st.caption(f"📋 Copy **{_kw}** above, then paste into GoodRx or OpenEvidence search")
