            ...
            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    ...
                    def clean_pa_snippet(text):
                        # Remove page header/footer lines e.g. "Y0040_ GHHJPMNES_C Updated 12/2025 Page 214 of 351"
                        text = re.sub(r'[A-Z0-9_]+\s+Updated\s+\d+/\d+\s+Page\s+\d+\s+of\s+\d+', '', text)
                        # Add newline before known section headers
                        headers = [
                            'PA Criteria', 'Criteria Details', 'Off-Label Uses', 'Exclusion',
                            'Criteria', 'Required', 'Medical Information', 'Age Restriction',
                            'Prescriber', 'Coverage', 'Duration', 'Other Criteria',
                            'Part B', 'Prerequisite', 'Restriction',
                        ]
                        for h in headers:
                            text = re.sub(r'(?<!\n)(' + re.escape(h) + r')', r'\n\1', text)
                        # Add newline before drug names in ALL CAPS followed by dash and form
                        text = re.sub(r'([A-Z]{3,}(?:\s+[A-Z]+)*\s+-\s+[A-Z]+)', r'\n\n\1', text)
                        # Collapse 3+ newlines
                        text = re.sub(r'\n{3,}', '\n\n', text)
                        # Clean up leading/trailing whitespace per line
                        lines = [l.strip() for l in text.split('\n')]
                        # Remove blank-ish lines that are just leftover fragments under 3 chars
                    ...
                    pa_criteria_link = (
                        ...
                        f'<div style="margin-top:8px;padding:12px 16px;background:#fffbf0;border-left:3px solid #e8913a;border-radius:0 6px 6px 0;font-size:0.82rem;line-height:1.8;color:#2a3a4a;max-height:350px;overflow-y:auto;">'
                        f'{snippet_html}'
