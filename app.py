...
def extract_pa(text, keyword):
    """Return True only if ALL keyword matches have PA nearby — any conflict returns False."""
    ...
    return pa_count == len(matches)


def extract_st(text, keyword):
    """Return True only if ALL keyword matches have ST nearby — any conflict returns False."""
    pattern = re.compile(re.escape(keyword.strip()), re.IGNORECASE)
    matches = list(pattern.finditer(text))
    if not matches:
        return False
