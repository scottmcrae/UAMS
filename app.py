...
def extract_tier(text, keyword):
    ...
    for m in pattern.finditer(text):
        ...
        # 2b. Dollar-amount tier format: "$0 (1)" or "$10 (2)" used by WellCare Dual plans
        dollar_tier = re.search(r'\$\d+\s*\(([1-6])\)', first_lines)
        if dollar_tier:
            return f"Tier {dollar_tier.group(1)}"

        # 3. Digit before PA/QL/ST flags - search across first 4 lines: "OZEMPIC ... 3 PA; QL"
        flag_match = re.search(r'\b([1-6])\s+(?:PA|QL|ST)\b', first_lines, re.IGNORECASE)
        if flag_match:
