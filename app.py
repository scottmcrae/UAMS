...
def _load_csv_plans():
    csv_path = Path("formulary_links.csv")
    ...
    with open(csv_path, newline="", encoding="utf-8-sig", errors="replace") as f:
        for row in _csv.reader(f):
            if len(row) >= 5 and row[4].strip().startswith("http"):
                name = row[3].strip()
                url  = row[4].strip()
                if "tricare" in name.lower():
                    if seen_tricare:
                        continue
                    seen_tricare = True
                    url = "https://www.express-scripts.com/frontend/open-enrollment/tricare/fst/#/"
                plans.append({"plan_name": name, "url": url})
    return sorted(plans, key=lambda x: x["plan_name"].lower())

if not (go and keyword.strip()):
    st.markdown("<div style='height:40vh'></div>", unsafe_allow_html=True)
with st.expander("View all formularies"):
    csv_plans = _load_csv_plans()
    display_plans = csv_plans if csv_plans is not None else sorted(entries, key=lambda e: e.get("plan_name","").lower())
    rows_html = "".join(
        f'<tr><td style="padding:7px 14px;border-bottom:1px solid #edf1f5;color:#0f1923;">{p.get("plan_name","")}</td>'
        f'<td style="padding:7px 14px;border-bottom:1px solid #edf1f5;"><a href="{p.get("url","")}" target="_blank" style="color:#416CA6;">{p.get("url","")}</a></td></tr>'
        for p in display_plans
    )
    st.markdown(
        f'<table style="width:100%;border-collapse:collapse;font-size:0.85rem;background:white;color:#0f1923;">'
        f'<thead><tr>'
        f'<th style="padding:10px 14px;text-align:left;border-bottom:2px solid #dde4eb;color:#416CA6;font-weight:600;font-size:0.78rem;text-transform:uppercase;letter-spacing:0.06em;">Plan</th>'
        f'<th style="padding:10px 14px;text-align:left;border-bottom:2px solid #dde4eb;color:#416CA6;font-weight:600;font-size:0.78rem;text-transform:uppercase;letter-spacing:0.06em;">URL</th></tr></thead>'
