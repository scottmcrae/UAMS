...
# Clear results when a new search term is submitted
if go and keyword.strip():
    if st.session_state.get("last_search","") != keyword.strip():
...
if go and keyword.strip():
    ...
    for _pa_plan_key, _pa_entries in sorted(_pa_entry_lookup.items()):
        # Skip if already shown in main results
        if _pa_plan_key in _seen_plans:
            continue
        # Check if any entry matches the drug term
        _best = None
        _best_score = 0
        ...
        if not _best:
            continue
        # Build PA criteria link for this plan
        _text = _best["text"]
        _section_start = None
        for _pat in [
            r'(?:^|\n)(' + re.escape(drug_term) + r'\s*\nFDA)',
        ...
        # Reconstruct original plan name from key for display
        _display_name = next((e["plan_name"] for e in _pa_entries if _normalize_plan(e["plan_name"]) == _pa_plan_key), _pa_plan_key.title())
        cards_html += (
            '<div class="result-card found">'
            f'<div class="plan-label"><span style="{_pa_only_style}">{_display_name}</span>'
            f' &nbsp; {_pa_link}</div>'
            '</div>'
        )
        _seen_plans.add(_pa_plan_key)
