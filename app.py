...
if go and keyword.strip():
    ...
    if found_results:
        st.markdown(f'<div style="background:#eaf2d7;border:1px solid #478000;border-radius:6px;padding:10px 16px;color:#478000;font-weight:600;">"{keyword.strip()}" found in <strong>{len(found_results)}</strong> formulary/formularies.</div>', unsafe_allow_html=True)
    else:
