        ...
        if is_tricare:
            ...
            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    ...
                    # Pattern that marks the start of a drug's PA section
                    # Handles: "Prior Authorization Group OZEMPIC" or ALL CAPS headings
                    section_header_pat = re.compile(
                        r'(?:Prior Authorization Group\s+' + re.escape(drug_term) + r')'
                        r'|(?:(?:^|\n)' + re.escape(drug_term.upper()) + r'(?:\s|$|\n))',
                        re.IGNORECASE
                    )
                    # Find the drug's own section header first
                    section_start_match = section_header_pat.search(pa_criteria_text)

                    if section_start_match:
                        start = section_start_match.start()
                        # Find the next drug's section header after this one
                        next_drug = re.search(
                            r'Prior Authorization Group\s+[A-Z]|(?:\n[A-Z][A-Z\s\-/,\(\)]{6,}\n)',
                            pa_criteria_text[section_start_match.end():]
                        )
                        end = section_start_match.end() + next_drug.start() if next_drug else min(len(pa_criteria_text), section_start_match.end() + 3000)
                        section = pa_criteria_text[start:end]
                    else:
                        # Fallback: use keyword match position
