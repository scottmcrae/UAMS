...
def format_freetext_html(text):
    ...
    # Strip UHC copyright line
    text = re.sub(
        r'©\s*\d{4}\s*UnitedHealthcare.*',
        '', text, flags=re.IGNORECASE | re.DOTALL
    ).strip()
    # Strip References section and everything below
    text = re.sub(
        r'\nReferences\n.*',
        '', text, flags=re.IGNORECASE | re.DOTALL
    ).strip()
    ...
    for line in lines:
        ...
        if not s:
            ...
            continue
        esc = _html.escape(s)
        # Count leading spaces to detect sub-indentation (caremark uses spaces)
        leading = len(raw) - len(raw.lstrip(' '))

        # Detect hierarchy by prefix
        if re.match(r'^\d+\.\s.{2,}:', s):                    # 1. Background: / 2. Coverage Criteria: — true top level
        ...
        elif re.match(r'^[A-Z]\.\s', s):                        # A. B. — section under top
            parts.append(f'<div style="margin:6px 0 2px 16px;font-weight:600;color:#1e3448;">{esc}</div>')
        elif re.match(r'^\d+\.\s', s):                         # 1. 2. — level 3
            parts.append(f'<div style="margin:4px 0 1px 32px;font-weight:600;">{esc}</div>')
        elif re.match(r'^[a-z]\.\s', s) and not re.match(r'^(are|and|or|the|of|to|in|is|for)\b', s):  # a. b.
            parts.append(f'<div style="margin:3px 0 1px 48px;">{esc}</div>')
        elif re.match(r'^\(\d+\)\s', s):                       # (1) (2) — level 5
            parts.append(f'<div style="margin:2px 0 1px 64px;">{esc}</div>')
        elif re.match(r'^\([a-z]\)\s', s):                     # (a) (b) — level 6
            parts.append(f'<div style="margin:2px 0 1px 80px;">{esc}</div>')
        elif re.match(r'^•\s', s) and leading < 4:             # top-level bullet
            rest = _html.escape(s[2:].strip())
            parts.append(f'<div style="display:flex;margin:3px 0 1px 24px;"><span style="flex-shrink:0;margin-right:6px;">•</span><span>{rest}</span></div>')
        elif re.match(r'^•\s', s) or leading >= 4:             # indented bullet / sub-item
            indent = 40 + min(leading, 12) * 4
            rest = _html.escape(s[2:].strip()) if s.startswith('• ') else esc
            parts.append(f'<div style="display:flex;margin:2px 0 1px {indent}px;"><span style="flex-shrink:0;margin-right:6px;">•</span><span>{rest}</span></div>')
        elif re.match(r'^◦\s', s):                             # direct ◦ sub-bullet
            rest = _html.escape(s[2:].strip())
            parts.append(f'<div style="display:flex;margin:2px 0 1px 48px;"><span style="flex-shrink:0;margin-right:6px;">◦</span><span>{rest}</span></div>')
            clean_esc = _html.escape(s[1:].strip())
            parts.append(f'<div style="display:flex;margin:2px 0 1px 48px;"><span style="flex-shrink:0;margin-right:6px;">◦</span><span>{clean_esc}</span></div>')
        ...
        elif re.match(r'^(Authorization of|Authorization will)', s, re.IGNORECASE):
            parts.append(f'<div style="margin:4px 0 2px 16px;font-style:italic;color:#555;">{esc}</div>')
        elif re.match(r'^©|^Reference number|Caremark\.|\.docx', s):
            parts.append(f'<div style="margin:2px 0;color:#aaa;font-size:0.72rem;">{esc}</div>')
        elif leading >= 4:                                      # space-indented continuation
                ...
                if _multi_entries:
                    ...
                    if best_entry:
                        ...
                        if _is_aetna_cpb:
                            ...
                            # Priority 1: "Indications" section
                            _ind_m = re.search(r'(?:^|\n)Indications\b', _text, re.IGNORECASE)
                            ...
                            _cont_m = re.search(r'(?:^|\n)Continuation of Therapy\b', _text, re.IGNORECASE)
                            if _ind_m:
                                _start = _ind_m.start()
                                _end = _cont_m.start() if (_cont_m and _cont_m.start() > _ind_m.start()) else min(len(_text), _start + 6000)
                            elif _cov_m2:
