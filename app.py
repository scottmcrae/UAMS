...
_pa_text_lookup = {}  # plan_name -> criteria text
if Path(PA_INDEX_FILE).exists():
    with open(PA_INDEX_FILE, encoding="utf-8") as _paf:
        _pa_index = json.load(_paf)
        for _pae in _pa_index.get("entries", []):
            if _pae.get("text") and _pae.get("status") == "ok":
                _pa_text_lookup[_pae["plan_name"]] = _pae["text"]
...
with st.form("search_form"):
    ...
    with col_search:
        keyword = st.text_input("Drug / keyword", placeholder="type the brand & plan  (tier results unreliable with generics)", label_visibility="collapsed")
    with col_btn:
...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        if is_tricare:
            ...
            pa_criteria_text = _pa_text_lookup.get(plan_label, "")

            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    # Strategy 1: Find "Prior Authorization Group DRUGNAME" anywhere in text
                    # This is the most reliable section marker
                    pa_group_pat = re.compile(
                        r'Prior Authorization Group\s+' + re.escape(drug_term) + r'\b',
                        re.IGNORECASE
                    )
                    pa_group_match = pa_group_pat.search(pa_criteria_text)

                    if pa_group_match:
                        start = pa_group_match.start()
                        # End at the next "Prior Authorization Group" entry
                        next_group = re.search(
                            r'Prior Authorization Group\s+(?!' + re.escape(drug_term) + r'\b)',
                            pa_criteria_text[pa_group_match.end():],
                            re.IGNORECASE
                        )
                        end = pa_group_match.end() + next_group.start() if next_group else min(len(pa_criteria_text), pa_group_match.end() + 3000)
                        section = pa_criteria_text[start:end]

                    else:
                        # Strategy 2: Find ALL CAPS drug class heading containing the drug
                        # e.g. "GLUCAGON-LIKE PEPTIDE-1 AGONISTS" section with "Ozempic" in Products Affected
                        kw_pat   = re.compile(re.escape(drug_term), re.IGNORECASE)
                        kw_match = kw_pat.search(pa_criteria_text)
                        if kw_match:
                            search_before = pa_criteria_text[:kw_match.start()]
                            search_after  = pa_criteria_text[kw_match.end():]
                            caps_headings = list(re.finditer(
                                r'(?:^|\n)([A-Z][A-Z0-9\s\-/,\(\)]{5,})(?:\n|$)',
                                search_before, re.MULTILINE
                            ))
                            start = caps_headings[-1].start() if caps_headings else max(0, kw_match.start() - 200)
                            next_boundary = re.search(
                                r'Prior Authorization Group\s+[A-Z]|(?:\n[A-Z][A-Z0-9\s\-/,\(\)]{5,}\n)',
                                search_after
                            )
                            end = kw_match.end() + next_boundary.start() if next_boundary else min(len(pa_criteria_text), kw_match.end() + 3000)
                            section = pa_criteria_text[start:end]
                        else:
                    ...
                    CATEGORIES = {
                        ...
                        'Required Medical Info':     [r'Required\s*Medical\s*Information', r'Required\s*Medical\s*Diagnosis', r'CriteriaRequired\s*Medical'],
                    ...
                    # Boundary-only labels: used to delimit values but not displayed
                    BOUNDARIES = [
                        r'Prescriber\s*Restrictions?',
                        r'Coverage\s*Duration',
                        r'CoverageDuration',
                        r'Authorization\s*will\s*be',
                        r'PA\s*Indication\s*Indicator',
                        r'Drug\s*Names?',
                        r'Products?\s*Affected',
                        r'Indications?\s*All',
                    ]

                    def extract_categories(text, categories, boundaries):
                        flat = re.sub(r'\s+', ' ', text)
                        # All patterns used as boundaries (categories + extra boundaries)
                        all_pats = '|'.join(
                    ...
                    rows = extract_categories(section, CATEGORIES, BOUNDARIES)

                    # Check if section is substantively empty (all dashes / no real content)
                    if rows:
                        rows_html = "".join(
                            f'<tr>'
                        ...
                        snippet_html = (
                            f'<div style="color:#888;font-size:0.8rem;font-style:italic;">No specific criteria listed for this drug — see full PDF for details.</div>'
                    ...
                    pa_criteria_link = (
                        f' <a href="{pa_criteria_url}" target="_blank" style="font-size:0.75rem;color:#856404;font-weight:500;text-decoration:none;border-bottom:1px dashed #856404;margin-left:10px;">PA Criteria ↗</a>'
                    )
            else:
                pa_criteria_link = ""
        ...
        plan_link = ('<a href="' + url + '" target="_blank" style="color:var(--ink);text-decoration:none;border-bottom:1.5px solid var(--teal);padding-bottom:1px;">' + plan_label + '</a>') if url else plan_label
        cards_html += (
            '<div class="' + card_class + '">'
            '<div class="plan-label">' + plan_link + ' &nbsp; ' + tag + tier_html + pa_html + st_html + pa_criteria_link + '</div>'
            '</div>'
        )
