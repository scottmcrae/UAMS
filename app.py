...
_pa_text_lookup = {}
_pa_index_path = Path("pa_index.json")
if _pa_index_path.exists():
    with open(_pa_index_path, encoding="utf-8") as _paf:
        for _pae in json.load(_paf).get("entries", []):
...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        if is_tricare:
            ...
            # Separate PDF link — always shown when PA URL exists and PA badge is present
            pa_pdf_link = (
                f' <a href="{pa_criteria_url}" target="_blank" style="font-size:0.75rem;color:#416CA6;font-weight:500;text-decoration:none;border-bottom:1px dashed #416CA6;margin-left:8px;">PA PDF ↗</a>'
                if (pa_criteria_url and has_pa) else ""
            )

            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    # Find relevant snippet around drug term
                    kw_pat    = re.compile(re.escape(drug_term), re.IGNORECASE)
                    kw_match  = kw_pat.search(pa_criteria_text)
                    if kw_match:
                        start   = max(0, kw_match.start() - 200)
                        end     = min(len(pa_criteria_text), kw_match.start() + 3000)
                        snippet = pa_criteria_text[start:end].strip()
                    else:
                        snippet = pa_criteria_text[:3000].strip()
                    snippet_html = snippet.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace('"',"&quot;").replace("\n","<br>")
                    expand_id = f"pac_{abs(hash(plan_label)) % 100000}"
                    pa_criteria_link = (
                        f' <a href="#" onclick="var d=document.getElementById(\'{expand_id}\');d.style.display=d.style.display===\'none\'?\'block\':\'none\';return false;" '
                        f'style="font-size:0.75rem;color:#856404;font-weight:500;text-decoration:none;border-bottom:1px dashed #856404;margin-left:10px;">PA Criteria ▾</a>'
                        f'<div id="{expand_id}" style="display:none;margin-top:8px;padding:12px 14px;background:#fffbf0;border-left:3px solid #e8913a;border-radius:0 6px 6px 0;font-size:0.78rem;line-height:1.7;color:#2a3a4a;white-space:pre-wrap;font-family:monospace;max-height:300px;overflow-y:auto;">'
                        f'{snippet_html}'
                        f'</div>'
                    )
                else:
        ...
        cards_html += (
            ...
            '<div class="plan-label">' + plan_link + ' &nbsp; ' + tag + tier_html + pa_html + st_html + pa_criteria_link + pa_pdf_link + '</div>'
            '</div>'
        )
