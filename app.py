...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        plan_link_with_underline = ('<a href="' + url + '" target="_blank" style="color:var(--ink);text-decoration:none;border-bottom:1.5px solid var(--teal);padding-bottom:1px;">' + plan_label + '</a>') if url else ('<span style="border-bottom:1.5px solid var(--teal);padding-bottom:1px;">' + plan_label + '</span>')

        if pa_criteria_link:
            # Plan has PA criteria — underline goes on PA Criteria, not the plan name
            top_row = '<div style="display:flex;align-items:center;flex-wrap:nowrap;gap:4px;">' + plan_link_no_underline + ' &nbsp; ' + tag + tier_html + pa_html + st_html + '</div>'
            ...
            # No PA criteria — keep underline on plan name
            top_row = '<div style="display:flex;align-items:center;flex-wrap:nowrap;gap:4px;">' + plan_link_with_underline + ' &nbsp; ' + tag + tier_html + pa_html + st_html + '</div>'
            _cards_list.append((plan_label.lower(), (
                '<div class="' + card_class + '">'
                '<div class="plan-label">'
                + top_row
    ...
    for _pa_plan_key, _pa_entries in _pa_entry_lookup.items():
        ...
        _pa_only_cards[_display_name.lower()] = (
            ...
            f'<div class="plan-label">'
            f'<div style="display:flex;align-items:center;flex-wrap:nowrap;gap:4px;"><span style="{_pa_only_style}">{_display_name}</span></div>'
            f'{_pa_link}</div>'
            '</div>'
        )
