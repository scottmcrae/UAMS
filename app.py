...
# Track which search mode is active across reruns
if go:
...
if (fda_go or _search_mode == "fda") and keyword.strip():
    ...
    with _results_placeholder.container():
        if not _fda_medguides:
            st.warning("`fda_medguides.json` not found — run `scrape_medguides.py` first.")
            ...
            st.markdown(
                ...
                f'FDA Medication Guides — {len(_fda_matches)} result{"s" if len(_fda_matches) != 1 else ""} '
            ...
            for _d in _fda_matches:
                ...
                _pdf_btn = (
                    ...
                    f'background:#023E73;color:white;padding:3px 12px;border-radius:6px;'
                    f'font-size:0.75rem;font-weight:600;text-decoration:none;'
                    f'white-space:nowrap;">&#128196; Open PDF</a>'
                ) if _url else ""
                ...
                _text_preview = ""
                if _text:
                    _preview_raw = _text[:600].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                    _text_preview = (
                ...
                _fda_cards += (
                    ...
                    f'<div class="plan-label" style="display:flex;align-items:center;'
            ...
            st.markdown(
                f'<div class="results-container">{_fda_cards}</div>',
                unsafe_allow_html=True,
            )
