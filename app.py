...
def _load_fda_medguides():
    # Look next to app.py, then fall back to cwd
    _here = Path(__file__).parent
    for _p in [_here / "fda_medguides.json", Path("fda_medguides.json")]:
        if _p.exists():
            with open(_p, encoding="utf-8") as _f:
                return json.load(_f)
    return []

_fda_medguides = _load_fda_medguides()
...
with st.form("search_form"):
    ...
    with col_btn2:
        fda_go = st.form_submit_button("📋 FDA Criteria", use_container_width=True)

if go:
    st.session_state["_search_mode"] = "formulary"
if fda_go:
    st.session_state["_search_mode"] = "fda"
...
# ── FDA Criteria results ───────────────────────────────────────────────────────
if (fda_go or _search_mode == "fda") and keyword.strip():
    _fda_term = keyword.strip().lower()
    _fda_matches = [
        d for d in _fda_medguides
        if _fda_term in d.get("drug_name", "").lower()
        or _fda_term in d.get("active_ingredient", "").lower()
    ]

    # Deduplicate by drug_name: keep most recent date, tie-break on first seen
    import re as _re
    from datetime import datetime as _dt
    def _parse_date(d):
        try:
            return _dt.strptime(d.get("date", ""), "%m/%d/%Y")
        except Exception:
            return _dt.min

    _fda_by_name = {}
    for _d in _fda_matches:
        _key = _d.get("drug_name", "").lower()
        if _key not in _fda_by_name or _parse_date(_d) > _parse_date(_fda_by_name[_key]):
            _fda_by_name[_key] = _d
    _fda_matches = list(_fda_by_name.values())

    with _results_placeholder.container():
        if not _fda_medguides:
            st.warning("`fda_medguides.json` not found — place it next to app.py.")
        elif not _fda_matches:
            st.error(f'❌ **"{keyword.strip()}"** not found in FDA Medication Guides.')
        else:
            st.divider()
            st.markdown(
                f'<p style="color:#416CA6;font-weight:600;margin-bottom:4px;">'
                f'FDA Medication Guides &mdash; {len(_fda_matches)} result{"s" if len(_fda_matches) != 1 else ""} '
                f'for <em>{keyword.strip()}</em></p>',
                unsafe_allow_html=True,
            )
            _fda_cards = ""
            for _d in _fda_matches:
                _dname = _d.get("drug_name", "")
                _ingr  = _d.get("active_ingredient", "")
                _form  = _d.get("form_route", "")
                _co    = _d.get("company", "")
                _date  = _d.get("date", "")
                _url   = _d.get("pdf_url", "")
                _text  = _d.get("medguide_text", "")

                _pdf_btn = (
                    f'<a href="{_url}" target="_blank" style="background:#023E73;color:white;'
                    f'padding:3px 12px;border-radius:6px;font-size:0.75rem;font-weight:600;'
                    f'text-decoration:none;white-space:nowrap;">&#128196; Open PDF</a>'
                ) if _url else ""

                _meta = " &nbsp;&middot;&nbsp; ".join(filter(None, [_ingr, _form, _co, _date]))

                _text_preview = ""
                if _text:
                    # First try: match the dashed section header
                    _ind_m = _re.search(r'-{5,}\s*INDICATIONS AND USAGE\s*-{5,}', _text, _re.IGNORECASE)
                    if not _ind_m:
                        # Fall back: find INDICATIONS AND USAGE not on a TOC line
                        for _m in _re.finditer(r'INDICATIONS AND USAGE', _text, _re.IGNORECASE):
                            _line_start = _text.rfind('\n', 0, _m.start()) + 1
                            _line_end   = _text.find('\n', _m.end())
                            _line = _text[_line_start:_line_end if _line_end > 0 else _m.end()+80]
                            if _re.search(r'\.{2,}|\(\s*1\s*\)', _line):
                                continue
                            _ind_m = _m
                            break
                    _display_text = _text[_ind_m.start():] if _ind_m else _text
                    # Truncate at DOSAGE AND ADMINISTRATION
                    _trunc_m = _re.search(r'-+\s*DOSAGE AND ADMINISTRATION\s*-+|-+\s*DOSAGE AND ADMINISTRATION|DOSAGE AND ADMINISTRATION', _display_text, _re.IGNORECASE)
                    if _trunc_m:
                        _display_text = _display_text[:_trunc_m.start()].strip()
                    # Also truncate at adverse reactions line as fallback
                    _adv_m = _re.search(r'To report SUSPECTED ADVERSE REACTIONS', _display_text, _re.IGNORECASE)
                    if _adv_m:
                        _display_text = _display_text[:_adv_m.start()].strip()
                    _preview_raw = _display_text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                    _text_preview = (
                        f'<div style="margin-top:8px;padding:10px 14px;background:#f8fafc;'
                        f'border-radius:6px;font-size:0.78rem;line-height:1.6;color:#1e3448;'
                        f'white-space:pre-wrap;max-height:1000px;overflow-y:auto;">'
                        f'{_preview_raw}</div>'
                    )
                ...
                _fda_cards += (
                    ...
                    f'&nbsp;{_pdf_btn}</div>'
                    f'<div class="payer-label" style="margin-top:2px;">{_meta}</div>'
                    f'{_text_preview}'
                    f'</div>'
                )
            st.markdown(f'<div class="results-container">{_fda_cards}</div>', unsafe_allow_html=True)
