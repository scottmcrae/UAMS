...
def format_freetext_html(text):
    ...
    # Strip embedded .docx filename references (appear mid-text in caremark PDFs)
    text = re.sub(r'\s*\S+\.docx\b', '', text, flags=re.IGNORECASE)
    ...
    for line in lines:
        ...
        if re.match(r'^\d+\.\s.{2,}:', s):                    # 1. Background: / 2. Coverage Criteria: — true top level
            ...
            parts.append(f'<div style="margin:3px 0 1px 24px;">{esc}</div>')
            ...
            if is_tricare:
                ...
                if _multi_entries:
                    ...
                    for _entry in _multi_entries:
                        ...
                        # drug term in header/title of text (first 300 chars) = +4
                        ...
                        _score = (_label_match * 2) + (_has_criteria * 3) + (len(_entry["text"]) > 2000) + (_header_match * 4)
                    ...
                    if best_entry:
                        ...
                        if _is_aetna_cpb:
                            ...
                            # Priority 1: find "Coverage Criteria" section — best starting point for UHC/narrative PDFs
                            ...
                            if _cov_m2:
                                ...
                                _end = min(len(_text), _cov_m2.start() + 6000)
                                ...
                                if _m:
                                    ...
                                    _end   = min(len(_text), _m.start() + 6000)
                                else:
                                    _start, _end = 0, 3000
