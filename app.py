...
def extract_tier(text, keyword):
    ...
    def _get_tier_from_match(after):
        ...
        # MO tier pattern: "sulfamethoxazole ... MO 2" — digit after MO flag
        mo_tier = re.search(r'\bMO\s+([1-6])\b', first_lines)
        if mo_tier:
        ...
        same_line = re.search(r'[^\S\n]{2,}([1-6])(?:\s*$|\s+(?:PA|QL|ST|MO)\b)', first_lines, re.IGNORECASE | re.MULTILINE)
        if same_line:
...
def extract_pa(text, keyword):
    """Return True only if ALL valid keyword matches have PA nearby."""
    ...
    if not valid_matches:
        return False
    pa_count = sum(
        1 for m in valid_matches
        if re.search(r'\bPA\b', '\n'.join(text[m.end():m.end()+300].split('\n')[:4]))
    )
    return pa_count == len(valid_matches)


def extract_st(text, keyword):
    """Return True only if ALL valid keyword matches have ST nearby."""
    pattern = re.compile(re.escape(keyword.strip()), re.IGNORECASE)
    valid_matches = []
    for m in pattern.finditer(text):
        line_start = text.rfind('\n', 0, m.start()) + 1
    ...
    st_count = sum(
        1 for m in valid_matches
        if re.search(r'\bST\b|step therapy', '\n'.join(text[m.end():m.end()+300].split('\n')[:4]), re.IGNORECASE)
    )
