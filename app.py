            ...
            if is_tricare:
                ...
                if _multi_entries:
                    ...
                    if best_entry:
                        ...
                        _drug_mention_count = len(re.findall(re.escape(drug_term), _text, re.IGNORECASE))
                        _is_ambiguous = False
                        if _ind_m_pre:
                            ...
                            if _pre_count > 1:
                                _is_ambiguous = True
                        elif _drug_mention_count > 3 and drug_term.lower() not in best_entry.get("drug_label","").lower():
                            # Drug mentioned many times but not as the primary subject — category document
                            _is_ambiguous = True
                        if _is_ambiguous:
