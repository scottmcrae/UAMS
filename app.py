...
def extract_tier(text, keyword):
    ...
    for m in pattern.finditer(text):
        ...
        # 2. Standalone digit (1-6) on the same line after the drug name
...
TRICARE_STATUS_MAP = {
    "NON-FORMULARY": ("Non-Formulary", "#fce4ec", "#c62828"),
    "BASIC CORE":    ("Covered",       "#e8f5e9", "#2e7d32"),
    "NOT COVERED":   ("Not Covered",   "#fff3e0", "#e65100"),
}

def extract_tricare_status(text, keyword):
    """Return list of unique Tricare coverage statuses found near keyword."""
    pattern = re.compile(re.escape(keyword.strip()), re.IGNORECASE)
    found = set()
    for m in pattern.finditer(text):
        snippet = text[m.start(): m.start() + 300]
        match = re.search(r'\b(NON-FORMULARY|BASIC CORE|NOT COVERED)\b', snippet)
        if match:
...
if go and keyword.strip():
    ...
    for r in (found_results + [] + []):
        ...
        # Tricare: show coverage status badges instead of tier
        if is_tricare and r.get("found") and r.get("text"):
            statuses = extract_tricare_status(r.get("text",""), keyword)
            labels = [TRICARE_STATUS_MAP.get(s, (s.title(), "", ""))[0] for s in statuses]
            tier_html = f'<span style="font-size:0.85rem;color:var(--muted);margin-left:8px;">{" · ".join(labels)}</span>' if labels else ""
        else:
            ...
            if tier:
                ...
                digit = re.search(r'(\d)', tier)
                bg, fg = tier_colors.get(digit.group(1), ('#f0f0f0','#444')) if digit else ('#f0f0f0','#444')
                tier_html = f'<span style="background:{bg};color:{fg};font-size:0.78rem;padding:3px 10px;border-radius:20px;margin-left:8px;font-weight:700;letter-spacing:0.02em;">{tier}</span>'
            else:
                tier_html = ""
        ...
        URL_OVERRIDES = {
            ...
            "tricare": "https://www.express-scripts.com/frontend/open-enrollment/tricare/fst/#/",
        }
