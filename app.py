        ...
        if is_tricare:
            ...
            if pa_criteria_url and has_pa:
                if pa_criteria_text:
                    ...
                    # Find the keyword in the PA text
                    ...
                    if kw_match:
                        ...
                        # Walk BACK to find the section start:
                        # Priority 1: "Prior Authorization Group DRUGNAME"
                        # Priority 2: ALL CAPS class heading (e.g. "GLUCAGON-LIKE PEPTIDE-1 AGONISTS")
                        pa_group = list(re.finditer(
                            r'Prior Authorization Group\s+' + re.escape(drug_term),
                            pa_criteria_text[:kw_match.start()], re.IGNORECASE
                        ))
                        ...
                        if pa_group:
                            start = pa_group[-1].start()
                        elif caps_headings:
                            start = caps_headings[-1].start()
                        else:
                            start = max(0, kw_match.start() - 200)

                        # Walk FORWARD to find section end:
                        # Next "Prior Authorization Group" or next ALL CAPS heading
