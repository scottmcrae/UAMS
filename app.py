...
def extract_tier(text, keyword):
    ...
    def _get_tier_from_match(after):
        ...
        # Single space before tier digit at end of line (Cigna/HealthSpring format)
        eol_tier = re.search(r'(?<!\d)(?<!\.)(?<!\-)\s([1-6])\s*$', line0)
        if eol_tier:
            return f"Tier {eol_tier.group(1)}"
...
def format_freetext_html(text):
    ...
    # Strip caremark boilerplate footer (line only, not everything after)
    text = re.sub(
        r'\n[^\n]*©\s*\d{4}\s*CVS Caremark[^\n]*',
        '', text, flags=re.IGNORECASE
    ).strip()
    # Strip "Caremark. All rights reserved." line
    text = re.sub(r'\n[^\n]*Caremark\.\s*All rights reserved\.[^\n]*', '', text, flags=re.IGNORECASE).strip()
    # Strip "This document contains confidential..." multi-line boilerplate
    text = re.sub(
        r'\nThis document contains confidential and proprietary information of CVS Caremark.*?CVS Caremark\.',
        '', text, flags=re.IGNORECASE | re.DOTALL
    ).strip()
    # Strip "pharmaceutical manufacturers that are not affiliated with CVS Caremark." line
    ...
    def _reflow_background(m):
        ...
        joined = re.sub(r'(?<![.!?*])\n(?=[a-zA-Z(])', ' ', body)
        # Split into sentences and prefix each with a bullet
    ...
    for line in lines:
        ...
        if re.match(r'^\d+\.\s.{2,}:', s):
            ...
            _last_was_bullet = False
        elif re.match(r'^•\s', s) and leading < 4:
            rest = _html.escape(s[2:].strip())
            _last_bullet_indent = 24
            parts.append(f'<div style="display:flex;margin:3px 0 1px {_last_bullet_indent}px;"><span style="flex-shrink:0;width:14px;">•</span><span>{rest}</span></div>')
            _last_was_bullet = True
        elif re.match(r'^•\s', s):
            ...
            _last_was_bullet = False
        elif re.match(r'^\u25cf\s', s):                          # background sentence bullet
            rest = _html.escape(s[2:].strip())
            parts.append(f'<div style="display:flex;margin:3px 0 1px 0;font-size:0.78rem;"><span style="flex-shrink:0;width:14px;">•</span><span>{rest}</span></div>')
            _last_was_bullet = False
...
# ── PA snippet extractor ─────────────────────────────────────────────────────
def extract_pa_snippet(text, drug_term):
    """Extract the relevant PA criteria snippet from free-form text.
    Returns (start, end) positions. Used by both main results and PA-only paths."""
    _bg_m   = re.search(r'(?:^|\n)\d+\.\s*Background\s*:', text, re.IGNORECASE)
    _ind_m  = re.search(r'(?:^|\n)Indications\s*\nFDA-[Aa]pproved Indications', text, re.IGNORECASE)
    _fda_m  = re.search(r'(?:^|\n)FDA-[Aa]pproved Indications', text, re.IGNORECASE)
    _cov_m  = re.search(r'(?:^|\n)(?:\d+\.\s*)?Coverage Criteria', text, re.IGNORECASE)
    _cont_m = re.search(r'(?:^|\n)(?:Continuation of Therapy|Criteria for continuation)\b', text, re.IGNORECASE)

    def _end_at(start, default_len=10000):
        return _cont_m.start() if (_cont_m and _cont_m.start() > start) else min(len(text), start + default_len)

    if _bg_m:
        return _bg_m.start(), _end_at(_bg_m.start())
    if _ind_m:
        return _ind_m.start(), _end_at(_ind_m.start())
    if _fda_m:
        return _fda_m.start(), _end_at(_fda_m.start())
    if _cov_m:
        return _cov_m.start(), _end_at(_cov_m.start())
    # Fall back to first keyword match
    _kw = re.search(re.escape(drug_term), text, re.IGNORECASE)
    if _kw:
        start = max(0, _kw.start() - 100)
        return start, _end_at(start)
    return 0, min(len(text), 3000)
...
def _load_ai_criteria():
    _here = Path(__file__).parent
    for _p in [_here / "ai_criteria.json", Path("ai_criteria.json")]:
        if _p.exists():
...
_ai_criteria = _load_ai_criteria()

with st.form("search_form"):
    col_search, col_btn, col_btn2 = st.columns([5, 1, 1.4])
    with col_search:
    ...
    with col_btn2:
        ai_go = st.form_submit_button("🤖 AI Criteria", use_container_width=True)

if go:
    st.session_state["_search_mode"] = "formulary"
if ai_go:
    st.session_state["_search_mode"] = "ai"
_search_mode = st.session_state.get("_search_mode", "formulary")
...
# ── AI Criteria results ────────────────────────────────────────────────────────
if (ai_go or _search_mode == "ai") and keyword.strip():
    _ai_term = keyword.strip().lower()
    _ai_matches = [
        d for d in _ai_criteria
        if _ai_term in d.get("DrugName", "").lower()
        or _ai_term in d.get("Generic", "").lower()
        or _ai_term in d.get("Class", "").lower()
    ]

    with _results_placeholder.container():
        if not _ai_criteria:
            st.warning("`ai_criteria.json` not found — place it next to app.py.")
        elif not _ai_matches:
            st.error(f'❌ **"{keyword.strip()}"** not found in AI Criteria.')
        else:
            st.divider()
            st.markdown(
                f'<p style="color:#416CA6;font-weight:600;margin-bottom:4px;">'
                f'AI Criteria &mdash; {len(_ai_matches)} result{"s" if len(_ai_matches) != 1 else ""} '
                f'for <em>{keyword.strip()}</em></p>',
                unsafe_allow_html=True,
            )
            _ai_cards = ""
            for _d in _ai_matches:
                _dname   = _d.get("DrugName", "")
                _generic = _d.get("Generic", "")
                _class   = _d.get("Class", "")

                # Support both schemas:
                # V2: fields nested under Section1_ClinicalSummary + Section2_AppealReadyNarrative
                # V4: fields at top level + AppealNarrative
                _s1 = _d.get("Section1_ClinicalSummary", {})
                def _get(key):
                    return _s1.get(key) or _d.get(key)

                _s2 = _d.get("Section2_AppealReadyNarrative") or _d.get("AppealNarrative", "")

                _content = ""

                _inds = _get("Indications") or []
                if _inds:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">Indications</div>'
                    for _item in _inds:
                        _content += f'<div style="margin:2px 0 1px 16px;font-size:0.78rem;color:#1e3448;">• {_item}</div>'

                _pac = _get("PACriteria") or []
                if _pac:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">PA Criteria</div>'
                    for _item in _pac:
                        _content += f'<div style="margin:2px 0 1px 16px;font-size:0.78rem;color:#1e3448;">• {_item}</div>'

                _st = _get("StepTherapyProtocols") or {}
                if _st:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">Step Therapy</div>'
                    for _level, _items in _st.items():
                        _content += f'<div style="margin:3px 0 1px 16px;font-size:0.78rem;font-weight:600;color:#416CA6;">{_level}</div>'
                        for _item in _items:
                            _content += f'<div style="margin:1px 0 1px 32px;font-size:0.78rem;color:#1e3448;">• {_item}</div>'

                _reauth = _get("ReauthorizationRequirements") or []
                if _reauth:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">Reauthorization</div>'
                    for _item in _reauth:
                        _content += f'<div style="margin:2px 0 1px 16px;font-size:0.78rem;color:#1e3448;">• {_item}</div>'

                _cov = _get("CoverageConsiderations") or {}
                if _cov:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">Coverage Considerations</div>'
                    for _plan, _items in _cov.items():
                        _content += f'<div style="margin:3px 0 1px 16px;font-size:0.78rem;font-weight:600;color:#416CA6;">{_plan}</div>'
                        for _item in (_items if isinstance(_items, list) else [_items]):
                            _content += f'<div style="margin:1px 0 1px 32px;font-size:0.78rem;color:#1e3448;">• {_item}</div>'

                _refs = _get("GuidelineReferences") or {}
                if _refs:
                    _content += '<div style="margin:8px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">References</div>'
                    for _num, _ref in _refs.items():
                        _rtitle = _ref.get("Title", "") if isinstance(_ref, dict) else str(_ref)
                        _rurl   = _ref.get("URL", "") if isinstance(_ref, dict) else ""
                        if _rurl:
                            _content += f'<div style="margin:2px 0 1px 16px;font-size:0.78rem;color:#1e3448;">&#8317;{_num}&#8318; <a href="{_rurl}" target="_blank" style="color:#416CA6;">{_rtitle}</a></div>'
                        else:
                            _content += f'<div style="margin:2px 0 1px 16px;font-size:0.78rem;color:#1e3448;">&#8317;{_num}&#8318; {_rtitle}</div>'

                if _s2:
                    _s2_esc = _s2.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                    _content += (
                        f'<div style="margin:10px 0 2px 0;font-weight:600;color:#1e3448;font-size:0.8rem;">Appeal-Ready Narrative</div>'
                        f'<div style="margin:4px 0 0 0;padding:10px 14px;background:#f0f4f8;border-radius:6px;'
                        f'font-size:0.78rem;line-height:1.6;color:#1e3448;">{_s2_esc}</div>'
                    )

                _ai_cards += (
                    f'<div class="result-card found" style="padding:10px 4px;">'
                    f'<div class="plan-label" style="display:flex;align-items:center;flex-wrap:wrap;gap:8px;">'
                    f'<span style="font-size:0.95rem;font-weight:700;color:#1e3448;">{_dname}</span>'
                    f'<span style="font-size:0.8rem;color:#6b7e8f;font-weight:400;">{_generic}</span>'
                    f'</div>'
                    f'<div class="payer-label" style="margin-top:2px;">{_class}</div>'
                    f'<div style="margin-top:8px;padding:10px 14px;background:#f8fafc;border-radius:6px;'
                    f'max-height:1000px;overflow-y:auto;">{_content}</div>'
                    f'</div>'
                )
            st.markdown(f'<div class="results-container">{_ai_cards}</div>', unsafe_allow_html=True)

elif go and keyword.strip():
    with _results_placeholder.container():
            ...
            if is_tricare:
                ...
                if _multi_entries:
                    ...
                    for _entry in _multi_entries:
                        ...
                        _score = (_label_match * 2) + (_has_criteria * 3) + (len(_entry["text"]) > 2000) + (drug_term.lower() in _entry["text"][:300].lower()) * 4
                        if _score > best_score:
                            best_score = _score
                            best_entry = _entry

                    if best_entry:
                        # If drug appears many times but isn't the primary subject — show link only
                        if drug_term.lower() not in best_entry.get("drug_label","").lower() and \
                           len(re.findall(re.escape(drug_term), best_entry["text"], re.IGNORECASE)) > 5:
                            ...
                            best_entry = None
                        # Skip if drug not in label and appears only incidentally
                        elif drug_term.lower() not in best_entry.get("drug_label","").lower() and \
                             len(re.findall(re.escape(drug_term), best_entry["text"], re.IGNORECASE)) <= 3:
                            best_entry = None
                            pa_criteria_link = ""
                            pa_criteria_url = ""  # prevent else branch from setting link
                    ...
                    if best_entry:
                        ...
                        if _is_aetna_cpb:
                            _fda_m = re.search(
                                r'(?:^|\n)' + re.escape(drug_term) + r'\b[^\n]{0,40}\n\s*FDA-approved Indications',
                                _text, re.IGNORECASE
                            )
                            _ind_header_m = re.search(r'(?:^|\n)Indications\s*\nFDA-[Aa]pproved Indications', _text, re.IGNORECASE)
                            _fda_header_m = re.search(r'(?:^|\n)FDA-[Aa]pproved Indications', _text, re.IGNORECASE)
                            _cov_m = re.search(r'(?:^|\n)Coverage Criteria', _text, re.IGNORECASE)
                            if _fda_m:
                                _start = _fda_m.start()
                                _cov_end = min(len(_text), (_cov_m.start() + 4000) if _cov_m else (_start + 5000))
                                # Stop before the next drug's FDA block
                                ...
                                _default_end = min(len(_text), (_cov_m.start() + 8000) if _cov_m else (_start + 8000))
                                _end = _cont_m.start() if (_cont_m and _cont_m.start() > _start) else _default_end
                            elif _fda_header_m:
                                _cont_m = re.search(r'(?:^|\n)(?:Continuation of Therapy|Criteria for continuation)\b', _text, re.IGNORECASE)
                                _start = _fda_header_m.start()
                                _default_end = min(len(_text), (_cov_m.start() + 8000) if _cov_m else (_start + 8000))
                                _end = _cont_m.start() if (_cont_m and _cont_m.start() > _start) else _default_end
                            elif _cov_m:
                                _start = _cov_m.start()
                            ...
                            # Priority 1: Background section
                            _bg_m = re.search(r'(?:^|\n)\d+\.\s*Background\s*:', _text, re.IGNORECASE)
                            # Priority 2: Indications\nFDA-approved Indications header
                            _start, _end = extract_pa_snippet(_text, drug_term)
                        ...
                        if pa_criteria_url and pa_criteria_text and drug_term.lower() in pa_criteria_text.lower():
                    ...
                    if pa_criteria_text:
                        ...
                        def _find_section(text, term):
                            ...
                            if kw:
                                # Prefer drug's own ALL-CAPS section header over previous drug's header
                                drug_header = re.search(r'(?:^|\n)' + re.escape(term.upper()) + r'\s*\n', text)
                                if drug_header:
                                    start = drug_header.start()
                                else:
                        ...
                        if section is not None and fmt != 'unknown':
                            ...
                            if fmt == 'bcbs':
                                ...
                                if req_cigna:
                                    req_val = 'Diagnosis'
                                elif req_split and _cv(req_split.group(1)).lower() not in ('medical','information','diagnosis',''):
                                    ...
                                    req_val = _cv(req_split.group(1) + ' ' + _continuation)
                                elif req_inline:
        ...
        for _pa_plan_key, _pa_entries in _pa_entry_lookup.items():
            ...
            for _pe in _pa_entries:
                ...
                if drug_term.lower() not in _hay:
                    continue
                _lm = drug_term.lower() in _pe["drug_label"].lower()
                _egonly = bool(re.search(r'\(e\.g\.[^)]*' + re.escape(drug_term), _pe["text"][:500], re.IGNORECASE)) and not _lm
                _score = (_lm * 2) + bool(_criteria_terms.search(_pe["text"])) * 3 + (drug_term.lower() in _pe["text"][:300].lower()) * 4 - (_egonly * 10)
                if _score > _best_score:
                    _best_score = _score
                    _best = _pe
            if not _best or _best_score <= 0:
                continue
            # Skip entirely if drug not in label and appears only incidentally
            if drug_term.lower() not in _best.get("drug_label","").lower() and \
               len(re.findall(re.escape(drug_term), _best["text"], re.IGNORECASE)) <= 3:
                continue
            _text = _best["text"]
            # If drug appears many times but isn't the primary subject — show link only
            if drug_term.lower() not in _best.get("drug_label","").lower() and \
               len(re.findall(re.escape(drug_term), _text, re.IGNORECASE)) > 5:
                _eu = _best.get("url","")
                _pa_link = f'<a href="{_eu}" target="_blank" style="font-size:0.88rem;color:#856404;font-weight:600;text-decoration:none;">PA Criteria ↗</a>' if _eu else ""
            else:
                _section_start, _end = extract_pa_snippet(_text, drug_term)
...
if "visit_counted" not in st.session_state:
    ...
    count_file = Path("visitor_count.txt")
    try:
...
st.markdown(
    ...
    <summary style="color:#ccc;font-size:0.65rem;cursor:pointer;list-style:none;user-select:none;">
    v1.0 © ARxive
    </summary>
    <span style="color:#888;font-size:0.75rem;">Visitors: {_get_visitor_count()}</span>
    </details>
    </div>''',
    unsafe_allow_html=True
)
