...
def format_freetext_html(text):
    ...
    for line in lines:
        ...
        if re.match(r'^\d+\.\s.{2,}:', s):                    # 1. Background: / 2. Coverage Criteria: — true top level
            ...
            parts.append(f'<div style="margin:3px 0 1px 0;padding-left:40px;text-indent:-16px;">{esc}</div>')
        elif re.match(r'^•\s', s) or leading >= 4:             # indented bullet / sub-item
            indent = 40 + min(leading, 12) * 4
            parts.append(f'<div style="margin:2px 0 1px 0;padding-left:{indent+16}px;text-indent:-16px;">{esc}</div>')
        elif re.match(r'^\uf0a7', s) or re.match(r'^\u25aa', s):  # Wingdings sub-bullet
            clean_esc = _html.escape(s[1:].strip())
            parts.append(f'<div style="margin:2px 0 1px 0;padding-left:64px;text-indent:-16px;">◦ {clean_esc}</div>')
